...
function buildPrompt(muscleGroups: MuscleGroup[], duration: number, intensity: WorkoutIntensity): string {
  ...
  const equipmentSection = gymProfile.equipment.length > 0
...
function parseNameList(text: string): string[] {
  ...
  const jsonStr = arrayMatch ? arrayMatch[0] : text.replace(/```json\s*/g, '').replace(/```\s*/g, '').trim();
...
export async function generateWorkoutPlanWithLLM(
  ...
  if (GEMINI_API_KEYS.length === 0) {
    return { plan: generateWorkoutPlan(goal, muscleGroups, duration, intensity), source: 'local' };
  }

  try {
    const text = await callGemini(buildPrompt(muscleGroups, duration, intensity));
    ...
    return { plan: { goal, muscleGroup: muscleGroups, duration, exercises }, source: 'llm' };
  } catch (err) {
