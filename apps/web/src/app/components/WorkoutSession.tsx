      ...
      <div className="px-5 pb-6">
        <div className="grid grid-cols-3 gap-2 text-center">
          <button
            ...
            aria-label={isResting ? 'Single tap, skip rest' : 'Single tap, complete set'}
            className={`flex flex-col items-center py-3 rounded-2xl bg-gradient-to-br from-blue-600 to-blue-700 hover:from-blue-500 hover:to-blue-600 transition-all shadow-lg shadow-blue-900/50 active:scale-[0.97] ${
          ...
          >
            ...
            <div className="mt-2 text-xs font-semibold">{isResting ? 'Skip rest' : 'Complete'}</div>
            <div className="text-[10px] text-blue-100/70">Single tap</div>
          </button>
          <button
            onClick={handleDoubleTap}
            aria-label="Double tap, skip exercise"
            className={`flex flex-col items-center py-3 rounded-2xl glass-dark hover:bg-white/10 transition-all shadow-lg active:scale-[0.97] ${
              flashTap === 'doubleTap' ? 'ring-4 ring-emerald-400 scale-[1.04]' : ''
            }`}
          >
            <div className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center gap-1">
              <div className="w-3 h-3 rounded-full bg-neutral-300" />
              <div className="w-3 h-3 rounded-full bg-neutral-300" />
            </div>
            <div className="mt-2 text-xs font-semibold">Skip exercise</div>
            <div className="text-[10px] text-neutral-500">Double tap</div>
          </button>
          <button
            onClick={handleTripleTap}
            aria-label="Triple tap, move exercise to end"
            className={`flex flex-col items-center py-3 rounded-2xl glass-dark hover:bg-white/10 transition-all shadow-lg active:scale-[0.97] ${
              flashTap === 'tripleTap' ? 'ring-4 ring-emerald-400 scale-[1.04]' : ''
            }`}
          >
            <div className="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center gap-0.5">
              <div className="w-2.5 h-2.5 rounded-full bg-orange-400" />
              <div className="w-2.5 h-2.5 rounded-full bg-orange-400" />
              <div className="w-2.5 h-2.5 rounded-full bg-orange-400" />
            </div>
            <div className="mt-2 text-xs font-semibold">Move later</div>
            <div className="text-[10px] text-neutral-500">Triple tap</div>
          </button>
        </div>
      </div>

      <ExerciseGuideSheet exercise={guideExercise} onClose={() => setGuideExercise(null)} />
      {showSetPlan && currentExercise && (
        <SetPlanSheet
          exercise={currentExercise}
