...
import { ArrowLeft, Play, Clock, Dumbbell, TrendingUp, Edit3, Trash2, Plus, Check, X, GripVertical, Minus, History, Volume2 } from 'lucide-react';
import type { WorkoutPlan, Exercise, ExerciseSet } from '../domain/workout';
import { getExerciseHistory, applyProgressiveOverload, getRecommendedSets } from '../utils/workoutHistory';
        ...
        <div className="space-y-3">
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => setEditForm({ ...editForm, restTime: parseInt(e.target.value) || 0 })}
              className="w-full px-3 py-2 bg-neutral-900 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-blue-600"
            />
          </div>
              ...
              {editForm.setDetails.map((set: ExerciseSet, idx: number) => (
                <div key={idx} className="grid grid-cols-[50px_1fr_1fr_40px] gap-2 items-center">
                  ...
                  <input
                    type="number"
                    value={set.weight}
                    onChange={(e) => onUpdateSetDetail(idx, 'weight', parseInt(e.target.value) || 0)}
          ...
          {exercise.setDetails && exercise.setDetails.length > 0 ? (
            <div className="space-y-2">
              ...
              <div className="text-center glass-dark rounded-xl p-2 mt-2">
                <div className="text-sm text-neutral-400">Rest: <span className="text-orange-400 font-semibold">{exercise.restTime}s</span></div>
              </div>
            </div>
          ) : (
...
export function PlanPreview({ plan, onStartSession, onBack }: PlanPreviewProps) {
  ...
  const handleEditExercise = (exercise: Exercise) => {
    ...
    const setDetails = exercise.setDetails || Array.from({ length: exercise.sets }, () => ({
      weight: 0,
      reps: exercise.reps,
      completed: false,
    }));
                    ...
                    {editForm.setDetails.map((set, idx) => (
                      <div key={idx} className="grid grid-cols-[50px_1fr_1fr_40px] gap-2 items-center">
                        ...
                        <input
                          type="number"
                          value={set.weight}
                          onChange={(e) => handleUpdateSetDetail(idx, 'weight', parseInt(e.target.value) || 0)}
                          className="px-3 py-2 bg-neutral-800 rounded-lg text-white text-center focus:outline-none focus:ring-2 focus:ring-blue-600"
                          placeholder="0"
                        />
                        <input
                          type="number"
                          value={set.reps}
                          onChange={(e) => handleUpdateSetDetail(idx, 'reps', parseInt(e.target.value) || 0)}
                          className="px-3 py-2 bg-neutral-800 rounded-lg text-white text-center focus:outline-none focus:ring-2 focus:ring-blue-600"
                          placeholder="0"
                        />
                        <button
          ...
          <div className="flex items-start gap-3">
            ...
            <div className="text-sm">
              ...
              <p className="text-blue-300/70">
                Audio guidance will guide you through each exercise. Use earbud taps to control your workout.
              </p>
            </div>
          </div>
        </div>
